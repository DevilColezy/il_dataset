...
void edt1d(std::vector<double>& f) {
    ...
    for (int q = 1; q < n; ++q) {
        ...
        // Guard against popping below the first envelope vertex (never read
        // v[-1]).
        while (k > 0 && s <= z[k]) {
            --k;
...
std::vector<double> squaredEdt3d(const std::vector<std::uint8_t>& seed,
    ...
    // Finite sentinel for non-seed cells (same reason as the observed-map
    // EDT): an +inf sentinel makes edt1d() produce s = -inf at a seed vs
    // inf envelope vertex and underflow k past 0 (OOB read of v[-1]).
