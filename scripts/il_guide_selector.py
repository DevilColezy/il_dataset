...
The production mode selects a Guide from only the current depth frame and the
relative mission goal.  It deliberately does not read future global-path
geometry, preventing stacked sensing horizons in Trend supervision.  The
legacy farthest-visible path selector remains available as an explicit
configuration mode for old datasets.
"""
...
class GuideSelector:
    ...
    Terminal aliases the selected Guide. Guide distance controls the speed
    envelope; the local optimizer supplies fine obstacle avoidance.
    """
    ...
    def select(self, global_path_world, previous_progress_index,
        ...
        """Select one Guide/Terminal using the configured mode.
        ...
        sel = GuideSelection(selection_mode="global_path_visible")
        ...
        # A Guide is a forward macro target, so its global-path index must not
        # move backward because one depth pixel flickered at the range/occlusion
        # boundary.  If neither the previous Guide nor a later waypoint is
        # currently visible, return no Guide and let the explicit RECOVERY
        # label handle that frame; never manufacture a backward NORMAL label.
        candidate_begin = min(
            max(start_index + 1, self._last_guide_path_index),
            len(global_path_world) - 1)
        ...
        if best_guide_idx < 0:
            sel.rejection_reason = (
                "no_monotonic_visible_waypoint"
                if self._last_guide_path_index >= 0
                else "no_visible_waypoint")
        ...
        self._last_guide_path_index = max(
            self._last_guide_path_index, best_guide_idx)
        sel.visible = True
        ...
        sel.guide_is_final = best_guide_idx >= len(global_path_world) - 1
        ...
        # ── Terminal compatibility fields (same point as Guide) ───────
        # Terminal aliases Guide. Guide distance constrains the current speed;
        # the optimized trajectory and ESDF provide fine obstacle avoidance.
        ...
        sel.terminal_path_index = best_guide_idx
        sel.terminal_distance_m = guide_dist
    ...
    def _check_depth_visibility(self, point_world, delta_flu, depth_m, dist):
        ...
        # Use the nearest valid surface in this footprint.  Taking the
        # farthest pixel would let one background pixel "see through" a
        # foreground obstacle.
        r_min = max(0, vi - 1)
        r_max = min(h - 1, vi + 1)
        c_min = max(0, ui - 1)
    ...
    def get_reference_segment(self, global_path_world, start_index, end_index):
        ...
        The local planner now uses straight-line initialization from current
        position to terminal, NOT a sub-path of the global reference.
