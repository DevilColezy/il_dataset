...
class MacroExpert(object):
    ...
    def update(self, goal_world, state, observed_map, dt_s=0.2,
        ...
        # Causal evidence (student-observable history only).
        self._update_causal_evidence(local_unrecoverable)

        # Rolling strategic progress (section VII) for DIRECT / SIDE.
        if self.mode in (self._module.MacroMode.DIRECT_GUIDE,
                         self._module.MacroMode.SIDE_GUIDE):
            self._update_progress(dt_s, goal_dist, state)

        # DIRECT no-progress watchdog -> FAILED.
        if self.mode == self._module.MacroMode.DIRECT_GUIDE and \
                self._no_progress_time_s > \
                self.cfg["max_no_progress_seconds"]:
        ...
        # SIDE no-progress (rolling) -> mark the committed side NO_PROGRESS
        # and fall through to macro intervention (try the other side).
        if self.mode == self._module.MacroMode.SIDE_GUIDE and \
                self._side_last_progress_time is not None:
            if now_s - self._side_last_progress_time > \
    ...
    # ── Causal evidence (section III) ────────────────────────────────
    def _update_causal_evidence(self, local_unrecoverable):
        # local_unrecoverable comes from the manager: repeated 30 Hz local
        # replanning failures / cached trajectory / BRAKE_HOLD.  Only this
        # student-observable history (never privilege alone) can upgrade
        # an observed-recoverable DIRECT into macro intervention.
        if local_unrecoverable:
            self._causal_local_failure_ticks += 1
        else:
            self._causal_local_failure_ticks = 0
        self.causal_intervention_evidence = \
            self._causal_local_failure_ticks >= \
            self.cfg.get("causal_evidence_frames", 2)

    # ── Rolling strategic progress (section VII) ─────────────────────
    def _update_progress(self, dt_s, goal_dist, state):
        now_s = time.monotonic()
        made_progress = False
        if self.mode == self._module.MacroMode.DIRECT_GUIDE:
            # DIRECT: goal-distance decrease (rolling, compared to prev).
            ...
            # SIDE: global cost-to-go decrease AND/OR arc-length progress
            # along the strategic path.  A temporary cost increase with
            # real forward travel is NOT a failure.
            pos = np.asarray(state["position"], dtype=np.float64)
    ...
    def _proceed_to_macro_intervention(self, goal, pos, yaw, direct_guide,
        ...
        # ── Side failure memory (section VIII) ───────────────────────
        self._update_side_failures(viable)
        ...
        # ── No viable candidate -> OBSERVE ───────────────────────────
        # OBSERVE side: committed (if not failed) or first non-failed or
        # LEFT (section VIII).  This naturally produces "OBSERVE LEFT" when
        # LEFT is not yet observable and RIGHT is a known dead-end.
        side = self._observe_side()
        self.committed_side = side

        # Rolling OBSERVE information-gain window (section VII): compare
        ...
        if self.mode != self._module.MacroMode.OBSERVE or \
            ...
            if known > self._observe_last_known or \
                ...
                self._observe_last_reachable_count = reachable
                self._observe_last_info_time = now_s
        # OBSERVE no-progress (rolling window): no new info for too long.
        if not info_gained and self._observe_last_info_time is not None and \
                now_s - self._observe_last_info_time > \
                self.cfg.get("observe_no_progress_seconds", 4.0):
            self.mode = self._module.MacroMode.FAILED
            self._failed_reason = "observe_no_progress"
            self._macro_decision_observable = False
            self._macro_decision_confidence = 0.0
            return self._build_action(
                self.mode, pos, None, False, 0.0, "observe_no_progress")

        # Sweep the yaw from the FIXED reference yaw captured at OBSERVE
        # entry.  Never current_yaw + accumulated delta.
    ...
    def _update_side_failures(self, viable):
        ...
        # LOCAL_PATH_FAILED (consecutive threshold) when a committed SIDE
        # loses its reachable path.
        if self.mode == self._module.MacroMode.SIDE_GUIDE:
            ...
            if not viable[committed]:
                ...
                if self._side_local_fail_count[committed] >= \
                    ...
                    self._mark_side_failed(committed,
                                           SideFailure.LOCAL_PATH_FAILED)
            else:
                self._side_local_fail_count[committed] = 0
        # OBSERVED_NO_CORRIDOR: the OBSERVE sweep saturated (full FOV
        # scanned) with still no usable corridor on the committed side.
        fov_half_obs = math.radians(self.cfg.get("observe_fov_half_deg", 45.0))
        if self.mode == self._module.MacroMode.OBSERVE and \
                self.committed_side != self._module.Side.NONE and \
                abs(self._observe_yaw_delta) >= fov_half_obs - 1e-6 and \
                not viable[self.committed_side]:
            self._mark_side_failed(self.committed_side,
    ...
    def _observe_side(self):
        ...
        if self._failed_right is None:
            return self._module.Side.RIGHT
        # Both sides failed: try the last committed first.
